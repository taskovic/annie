...
export function MultipleFileUploadField({ name }: { name: string }) {
    ...
    const onDrop = useCallback((accFiles: File[], rejFiles: FileRejection[], event: DropEvent) => {
    ...
    const getDropzoneClassName = (active: boolean) => {
        if (active)
            return "annie-dropzone active-dropzone"
        else
            return "annie-dropzone"
    }
        ...
        <>
            <div {...getRootProps({ className: getDropzoneClassName(isDragActive) })}>
                ...
                <div className="annie-dropzone-text">
                    <div><span>Click to upload</span><p>&nbsp;or drag and drop</p></div>
